...
function tokenize(str) {
  ...
  var err = function(msg) {
    throw new Error('Error at row ' + row + ', col ' + col + ' (character ' + str[pos] + ')' + ": " + msg)
  }
  ...
  while (pos < str.length) {
        ...
        if(str[pos+ind] === '\n') {
          ...
          if (ind > indent[indent.length-1]) {
            ...
            if(indent.length > 0 && (indent[indent.length-1] != ind)) {
              err('indentation mismatch: ' + indent[indent.length-1]  + ' vs ' + ind)
            }
          }
          pos += ind
    ...
    if (state === TYPE) {
      ...
      } else if (s === '(') consume('TYPE', INLINE_PARAMS)
      else if (s === '\n') consume('TYPE', CONTENT)
      ...
      if (s === '@') consumeIfNonEmpty('CONTENT', TYPE)
      else if (s === '\n') {
