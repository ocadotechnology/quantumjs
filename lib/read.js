...
function inline (parsed, currentDir, options) {
  var promises = []
  ...
  parsed.content.forEach(function(entity, i) {
    if(entity.type === options.inlineEntityType) {
      ...
      var promise = parseFiles(filename, doParse, options)
        .then(function(res){
      ...
      promises.push(inline(entity, currentDir, options))
    }
  })
...
function parseFiles (globString, doParse, options) {
  return glob(globString).map(function(filename) {
    return parseFile(filename, doParse, options)
  })
}
...
function readSingle (filename, options) {
  ...
  if(options.inline) {
    return enhancePromise(parseFile(filename, true, options)
      .then(function(res){
        return {
    ...
    return enhancePromise(options.loader(filename)
      .then(parse)
      .then(function(res){
        return {
          filename: relativeFilename,
...
function enhancePromise (promise) {
  ...
  promise.map = function(f) {
    return oldMap.call(promise, f).then(flatten)
  }
  return promise
}

module.exports.single = readSingle
